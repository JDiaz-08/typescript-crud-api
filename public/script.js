...
async function handleRegister(e) {
  ...
  const res = await apiFetch('/auth/register', {
    ...
    body: JSON.stringify({
      ...
      password, confirmPassword: password, role: 'User',
    }),
  });
  if (!res) return;
...
async function handleAccountSubmit(e) {
  ...
  const body = editId
    ...
    : { title: 'Mr', firstName, lastName, email,
        password, confirmPassword: password, role, verified };
...
async function deleteAccount(id) {
  ...
  if (!res) return;
  res.ok
    ? (showToast('Account deleted', 'info'), renderAccountsList())
...
async function renderDepartmentsList() {
  ...
  departments.forEach(dept => {
    // Escape single quotes in name/description to avoid breaking onclick attributes
    const safeName = (dept.name || '').replace(/'/g, "\\'");
    const safeDesc = (dept.description || '').replace(/'/g, "\\'");
    html += `
      <tr>
        <td>${dept.name}</td>
        ...
        <td class="table-actions">
          <button class="btn btn-sm btn-primary"
            onclick="editDepartment(${dept.id}, '${safeName}', '${safeDesc}')">Edit</button>
...
async function deleteDepartment(id) {
  ...
  if (!res) return;
  res.ok
    ? (showToast('Department deleted', 'info'), renderDepartmentsList())
    : showToast('Delete failed', 'danger');
}
...
async function showEmployeeForm(employeeId = null) {
  ...
  document.getElementById('emp-user').innerHTML =
    users.map(u =>
  ...
  if (employeeId) {
    ...
    if (res && res.ok) {
      ...
      document.getElementById('emp-user').value       = emp.userId;
    ...
    form.reset();
    // Re-populate dropdowns after reset clears them
    document.getElementById('emp-department').innerHTML =
      depts.map(d => `<option value="${d.id}">${d.name}</option>`).join('');
    document.getElementById('emp-user').innerHTML =
      users.map(u =>
...
async function handleEmployeeSubmit(e) {
  ...
  const userId       = Number(document.getElementById('emp-user').value);
...
async function deleteEmployee(id) {
  ...
  if (!res) return;
  res.ok
    ? (showToast('Employee deleted', 'info'), renderEmployeesList())
    : showToast('Delete failed', 'danger');
}
...
async function renderRequestsList() {
  ...
  const url = currentUser?.role === 'Admin' ? '/requests/all' : '/requests';
  ...
  if (requests.length === 0) {
    document.getElementById('requests-list').innerHTML = `
      <div class="alert alert-info">No requests found.<br>
      ...
      <tr>
        ...
        ${isAdmin ? `
          <td class="table-actions">
            ...
            <button class="btn btn-sm btn-danger"
              onclick="updateRequestStatus(${req.id}, 'Rejected')">Reject</button>
            <button class="btn btn-sm btn-secondary"
              onclick="updateRequestStatus(${req.id}, 'Pending')">Pending</button>
          </td>` : ''}
      </tr>`;
  });
...
async function updateRequestStatus(id, status) {
  ...
  const data = await res.json();
  res.ok
    ? (showToast(`Request marked as ${status}`, 'success'), renderRequestsList())
